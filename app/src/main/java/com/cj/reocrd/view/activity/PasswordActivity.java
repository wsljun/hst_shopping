...
public class PasswordActivity extends BaseActivity<MyPrresenter> implements MyContract.View {
    ...
    VerificationCodeView passowrdPassword;
    ...
    VerificationCodeView passowrdPassword2;
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.password_next2:
                if(TextUtils.isEmpty(passowrdPassword.getInputContent())){
            ...
            case R.id.password_next3:
                if(TextUtils.isEmpty(passowrdPassword2.getInputContent())){
                    ToastUtil.showToastS(mContext, "请再次确认密码");
                    return;
                }
                if(!passowrdPassword2.getInputContent().equals(passowrdPassword.getInputContent())){
                    ToastUtil.showToastS(mContext, "两次密码不一致，请重新输入");
                    return;
                }
                type = 2;
                mPresenter.updatePwd("110",uid,passwordCode.getText().toString(),passowrdPassword.getInputContent());
                break;
